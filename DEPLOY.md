...
シフト表はGoogleスプレッドシートURLから直接読み込みます。URLに`gid`が含まれていればそのタブを、含まれていなければ先頭の表示タブを対象にします。

「作成結果を反映」は元タブを複製し、新しい作成結果タブへシフトくんが作成したセルだけを書き込みます。元のタブは変更しないため、書式、文字色、非表示行、固定セルを維持したまま確認できます。

このプロジェクトでは以下のAPIを有効化済みです。

- Google Sheets API
- Service Usage API

初回の読み込み時に、URLで指定したスプレッドシートを読み取り、新しいタブを追加する権限を求められます。
