...
「スプレッドシートへ出力」は、読み込んだExcelファイルをGoogleスプレッドシートへ変換し、シフトくんが作成したセルだけを書き込みます。元の文字色、非表示行、固定セルは保持されます。
...
初回出力時のみ、出力先のGoogleドライブにファイルを作成する許可を求められます。アプリが新規作成する出力ファイルだけにアクセスする最小権限を使用します。
